  ...
  )
{
  ...
  return ((UINT64)MMC_SYS_PLL_RAT (RcwSr) * SysClkHz) >> 1;
}
  ...
  )
{
  ...
  MmcClk = GetEsdhcClockHz ();
  if (!MmcClk) {
    DEBUG ((DEBUG_ERROR, "Invalid Mmc clock\n"));
    return EFI_SUCCESS;
  }

  MmcClkBe = CpuToFdt32 ((UINT32)MmcClk);
  FdtStatus = FdtSetProp (Dtb, NodeOffset, "clock-frequency", &MmcClkBe, sizeof (MmcClkBe));
  if (FdtStatus) {
  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    ...
    DEBUG ((
      DEBUG_ERROR,
      "MmcHostDxe: installed EmbeddedMmcHost handle=%p cardType=%d base=0x%Lx\n",
      Handle,
      MmcHostInstance->CardType,
      (UINT64)MmcHostInstance->DeviceBaseAddress
      ));
  }
