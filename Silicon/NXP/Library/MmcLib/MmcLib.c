  ...
  )
{
  ...
  DEBUG ((
    DEBUG_ERROR,
    "SetSysctl(): request=%u sdhc=%d actual=%d prediv=%d div=%d clk=0x%08x\n",
    Clock,
    SdhcClk,
    mMmc->Clock,
    PreDiv,
  ...
  Timeout = 20;
  while (!(MmcRead ((UINTN)&Regs->Prsstat) & PRSSTATE_SDSTB) && --Timeout) {
    MicroSecondDelay (1000);
  }

  if (Timeout <= 0) {
    DEBUG ((DEBUG_ERROR, "SetSysctl(): SD clock never stabilised prsstat=0x%x sysctl=0x%x\n",
      MmcRead ((UINTN)&Regs->Prsstat),
      MmcRead ((UINTN)&Regs->Sysctl)
      ));
  }
  ...
  )
{
  ...
  // U-Boot clears TBCTL[TB_EN] here because reset-all does not reliably do it.
  MmcAnd ((UINTN)&Regs->Tcr, ~TBCTL_TB_EN);
